...
export default function AppendBountyButton({
  ...
  const { appendBountyMutation } = useBounty();
  ...
  const handleAppend = async () => {
    ...
    await appendBountyMutation.mutateAsync({
      bountyId,
      amount: Number(appendAmount),
    });
    closeAppendModal();
    // 如果有成功回调函数，则调用
    if (onSuccess) {
          ...
          <div className="bg-white p-6 rounded-2xl shadow-lg w-96 border border-[#E0E5E3]">
            <div className="flex justify-between items-center mb-4 pb-2 border-b border-[#E0E5E3]">
              ...
              <button
                ...
                disabled={ appendBountyMutation.isPending}
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="mb-5">
              ...
              <p className="mt-2 text-sm text-[#8CA29F]">
                追加后的积分将增加到悬赏总额中
              </p>
            </div>
            ...
            <div className="flex justify-end space-x-3">
              ...
              <button
                ...
                className={`px-4 py-2 cursor-pointer bg-gradient-to-r from-[#5E8B7E] to-[#4F7A6F] text-white rounded-lg hover:from-[#4F7A6F] hover:to-[#3D685F] transition-colors shadow-md flex items-center gap-1.5 ${
                  !appendAmount || appendBountyMutation.isPending
                    ? "opacity-50 cursor-not-allowed"
                    : ""
                }`}
                disabled={!appendAmount ||  appendBountyMutation.isPending}
              >
                { appendBountyMutation.isPending ? (
                  <>
